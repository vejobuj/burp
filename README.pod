...
I<burp> [options] PACKAGE [PACKAGE2..]
...
=item B<-u> I<USER>B<, --username=>I<USER>
...
Use I<FILE> to store cookies rather than the default temporary file. If a valid cookie
file is found, burp will attempt to login using the cookies in this file rather than
by username and password. Do B<not> attempt to use a cookie file from your web browser
or other application.

=item B<-k, --keep-cookies>

Cookies will be persistent and reused for logins. Do B<not> use this option without
specifying a path to a cookie file, either by the -C option or via the config file.
